...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const btnModeTextured =
    document.getElementById('btn-mode-textured');

  const btnModeWireframe =
    document.getElementById('btn-mode-wireframe');

  const btnModePoints =
    document.getElementById('btn-mode-points');

  const btnToggleGrid =
    document.getElementById('btn-toggle-grid');

  const btnToggleBBox =
    document.getElementById('btn-toggle-bbox');

  const btnResetCam =
    document.getElementById('btn-reset-cam');
  ...
  if (viewer3dController) {
    if (
      ...
      btnModePoints.addEventListener('click', () => {
        ...
        viewer3dController.loadModel(
          '/api/reconstruction/sparse',
          'ply'
        );
      });
    }
  ...
  const valTotalImages =
    document.getElementById('val-total-images');

  const valGpsCoverage =
    document.getElementById('val-gps-coverage');

  const valRtkStatus =
    document.getElementById('val-rtk-status');

  const valGcpCount =
    document.getElementById('val-gcp-count');

  const datasetTag =
    document.getElementById('dataset-location-tag');

  const filterSearch =
    document.getElementById('filter-search');

  const filterGps =
    document.getElementById('filter-gps');

  const filterRtk =
    document.getElementById('filter-rtk');

  const filterQuality =
    document.getElementById('filter-quality');
  ...
  const btnViewTable =
    document.getElementById('btn-view-table');

  const btnRefresh =
    document.getElementById('btn-refresh');

  const btnFitBounds =
    document.getElementById('btn-fit-bounds');

  const toggleTrajectory =
    document.getElementById('toggle-trajectory');

  const toggleGcps =
    document.getElementById('toggle-gcps');

  const toggleHeading =
    document.getElementById('toggle-heading');
  ...
  function generateDemoRecords() {
    ...
    for (let i = 1; i <= 500; i++) {
      records.push({
        ...
        filename: `DJI_${String(i).padStart(4, '0')}.JPG`,
        gps_available: true,
        gps: true,
        rtk_available: true,
        rtk: true,
        quality: i % 17 === 0 ? 'Good' : 'Excellent',
        latitude: 16.7050 + (i * 0.00002),
        longitude: 74.2433 + (i * 0.00002),
        altitude: 120 + (i % 20),
        heading: i % 360,
        gcp: i <= 5
      });
    ...
    document.querySelectorAll('*').forEach((el) => {
      if (
        ...
        el.textContent
          .trim()
          .toLowerCase()
          .includes('ingesting and analyzing dataset')
      ) {
  ...
  function loadDemoData() {
    ...
    if (valTotalImages) {
      valTotalImages.textContent = '500';
    }
    ...
    if (galleryController) {
      galleryController.setRecords(records);
    }
  ...
  async function loadData() {
    ...
    try {
      ...
      const imagesResp =
        await fetch('/api/images?limit=500');
